...
$$\begin{split}
  d(x,y) & = \left[ \frac{w_1\delta_{x_1y_1}^k}{\sum_{k=1}^n w_k\delta_{x_iy_i}^k}\right] d_{x_1y_1}^1 + \ldots + \left[ \frac{w_n\delta_{x_ny_n}^k}{\sum_{k=1}^n w_k\delta_{x_iy_i}^k} \right] d_{x_ny_n}^n \\ 
...
$$d_{x_ky_k}^k = 
  \begin{cases}
   0 & \text{if } x_k, y_k \text{ is a nominal or binary variable and } x_k = y_k \\
   1 & \text{if } x_k, y_k \text{ is a nominal or binary variable and } x_k \neq y_k \\
...
iteration_2_tbl <- iteration_1_tbl |> 
  tab_style(
    style = list(
...
iteration_3_tbl <- iteration_2_tbl |> 
  tab_style(
    style = list(
...
iteration_4_tbl <- iteration_3_tbl |> 
  tab_style(
    style = list(
      cell_fill(color = "#CCBE93"),
      cell_text(color = "white")
    ),
    locations = cells_body(
      columns = 5,
...
```{r}
set.seed(seed = 1234) # By default R chooses at random the initial centers
                      # with set.seed we always fix the same initial centers
...
n_clusts_kaufman_example <- kaufman_example |> 
  ...
  n_clusters(
    ...
    n_max = 7 # We have only 8 rows
  )

# select the most parsimonious solution with fewer clusters
n_clusts_kaufman_example

# show the criterions used
n_clusts_kaufman_example |> 
  as_tibble()
```
...
-   Select a number of clusters, using `segmentation`

    -   Use the package `parameters` to select the number of clusters for k-means
...
-  Choose the numbers of clusters using the package `parameters` for k-means

```{r}
n_clusts_segmentation_numeric_scale <- segmentation_numeric_scale |> 
  n_clusters(
    standardize = FALSE, # In this case we don't need to standardize
    nbclust_method = "kmeans",
    n_max = 10 # This is the number set by default
  )

n_clusts_segmentation_numeric_scale
```

## 

- You can check, if you want, the criteria used to select the number of clusters:
...
-   Apply k-means with $k = 5$ and [**Hartigan-Wong algorithm**](https://en.wikipedia.org/wiki/K-means_clustering)

    -   k-means start with $k = 5$ random centers so you need to fix this initial decision using `set.seed` if the clusters tend to change
...
[**cluster**](https://CRAN.R-project.org/package=cluster) and [**tinytex**](https://CRAN.R-project.org/package=tinytex) for allowing me to access these tools without paying for a license
